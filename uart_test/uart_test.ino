...
void loop() {
  ...
  Serial.print(F("Arduino Core For ESP8266 Version: "));    //Serial.print(F(xx))    不换行，F()占用rom，不占用运行内存ram
  Serial.println(coreVersion);                              //Serial.println(xx)   串口打印xx后换行
  Serial.printf("Flash real id（唯一标识符）: %08X\n", ESP.getFlashChipId());   //Serial.printf(xx)可以进行格式化输出
